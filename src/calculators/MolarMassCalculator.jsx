...
export default function MolarMassCalculator({ onClose, onStatusUpdate }) {
  ...
  const handleCalculate = () => {
    ...
    try {
      ...
      const M = parseFloat(molarity || '1.0');
      const V = parseFloat(volume || '0.5');
      const MW = parseFloat(molecularWeight || '58.44');

      if (isNaN(M) || M <= 0) throw new Error('Molarity must be a positive number.');
      if (isNaN(V) || V <= 0) throw new Error('Volume must be a positive number.');
      if (isNaN(MW) || MW <= 0) throw new Error('Molecular weight must be a positive number.');
      ...
      const formattedMolarity = formatWithSIPrefix(M, 'M', 4);
      const formattedVolume = formatWithSIPrefix(V, 'L', 4);
    ...
    <div>
      ...
      {result && (
        <div className="result">
          ...
          <br />
          For {result.molarity} solution in {result.volume}
          <br />
